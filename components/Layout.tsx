...
export const Layout: React.FC = () => {
  ...
  const navItems = [
    { name: 'Panel de Control', path: '/', icon: Icons.Dashboard, hasSubmenu: false, show: true },
    { name: 'Contratos', path: '/contracts', icon: Icons.FileText, hasSubmenu: true, show: permissions.access_contract_generator },
    { name: 'Documentos', path: '/documents', icon: Icons.Contracts, hasSubmenu: false, show: permissions.can_view_all_documents }, // New Item
    { name: 'Clientes', path: '/clients', icon: Icons.Clients, hasSubmenu: true, show: true },
    { name: 'Tareas', path: '/tasks', icon: Icons.Tasks, hasSubmenu: false, show: true },
    { name: 'Calendario', path: '/calendar', icon: Icons.Calendar, hasSubmenu: false, show: permissions.access_calendar },
    { name: 'Equipo', path: '/team', icon: Icons.User, hasSubmenu: false, show: permissions.access_team_management },
  ];

  const bottomItems = [
    { name: 'Configuraciones', path: '/settings', icon: Icons.Settings, show: permissions.access_settings },
    { name: 'Soporte', path: '/support', icon: Icons.Help, show: true },
  ];
      ...
      `}>
        ...
        <div className="p-3 mt-auto">
          ...
          <div className="mt-2 flex items-center gap-3 px-3 py-2 rounded-md hover:bg-[#EFEFEF] cursor-pointer transition-colors">
            ...
            <div className="text-[13px] font-medium text-gray-700">Juan Perez</div>
          </div>
        </div>
      </aside>
