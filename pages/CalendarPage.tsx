...
export const CalendarPage: React.FC = () => {
    ...
    const handleDeleteEvent = () => {
        if (selectedEvent?.id && confirm('¿Eliminar evento?')) {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0">
                <div className="flex items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 text-sm mt-1 capitalize">{currentMonthName}</p>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <button onClick={() => setCurrentDate(new Date())} className="px-3 text-xs font-semibold text-gray-700 hover:bg-gray-50">Hoy</button>
                    ...
                    {!isGoogleConnected ? (
                        ...
                        <span className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-700 text-xs font-bold rounded-lg border border-green-100">
                            <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div> Google Sincronizado
                        </span>
                    )}
                    ...
                    <button onClick={handleCreateEvent} className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-800 shadow-sm">
                        <Icons.Plus size={16} /> Nuevo Evento
                    </button>
                </div>
            </div>
                                    ...
                                    <div className="flex-1 p-2 space-y-2 overflow-y-auto">
                                        {dateItems.length === 0 && (
                                            <div className="h-full flex items-center justify-center border-2 border-dashed border-transparent hover:border-gray-200 rounded-lg transition-all">
                                                <p className="text-[10px] text-gray-300 font-medium">Suelte aquí</p>
                                            </div>
                                        )}
                                        {dateItems.map(item => renderCalendarItem(item))}
                                    </div>
                    ...
                    <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSaveEvent} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input name="location" defaultValue={selectedEvent.location} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Agregar ubicación..." />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Descripción</label>
                                <textarea name="description" rows={3} defaultValue={selectedEvent.description} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-none" placeholder="Notas..." />
                            </div>
                            <div className="flex justify-between pt-4 border-t border-gray-100 mt-2">
                                {selectedEvent.id && <button type="button" onClick={handleDeleteEvent} className="text-red-600 text-sm font-medium hover:underline">Eliminar Evento</button>}
                                <div className="flex gap-2 ml-auto">
                                    <button type="button" onClick={() => setIsEventModalOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
                                    <button type="submit" className="px-4 py-2 text-sm bg-gray-900 text-white rounded-lg hover:bg-gray-800">Guardar</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="bg-white rounded-xl w-full max-w-sm p-6 shadow-xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSaveTask} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="status" defaultValue={selectedTask.status} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                        {Object.values(TaskStatus).map(s => (
                                            <option key={s} value={s}>{s === TaskStatus.TODO ? 'Por Hacer' : s === TaskStatus.IN_PROGRESS ? 'En Progreso' : s === TaskStatus.ON_HOLD ? 'En Espera' : 'Completada'}</option>
                                        ))}    </select>
                                </div>
                                <div>
                                    ...
                                    <select name="priority" defaultValue={selectedTask.priority} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                        {Object.values(TaskPriority).map(s => <option key={s} value={s}>{s === 'Urgent' ? 'Urgente' : s === 'High' ? 'Alta' : 'Media'}</option>)}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-2 pt-4 border-t border-gray-100 mt-2">
                                <button type="button" onClick={() => setIsTaskModalOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
                                <button type="submit" className="px-4 py-2 text-sm bg-green-700 text-white rounded-lg hover:bg-green-800">Guardar Tarea</button>
                            </div>
                        </form>
                    </div>
                </div>
