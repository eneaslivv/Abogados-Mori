...
export const Tasks: React.FC = () => {
    ...
    const Columns = [
        ...
        { title: 'Completadas', status: TaskStatus.COMPLETED },
    ];
        ...
        <div className="h-full flex flex-col relative overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <Icons.Plus size={16} /> Nueva Tarea
                    </button>
                </div>
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${task.priority === TaskPriority.URGENT ? 'bg-red-100 text-red-700' :
                                                task.priority === TaskPriority.HIGH ? 'bg-orange-100 text-orange-700' :
                                                    'bg-blue-100 text-blue-700'
                                                }`}>
                                                {task.priority === 'Urgent' ? 'Urgente' : task.priority === 'High' ? 'Alta' : task.priority === 'Low' ? 'Baja' : 'Media'}
                                            </span>
            ...
            {isDrawerOpen && selectedTask && (
                <div className="absolute inset-y-0 right-0 w-[480px] bg-white/95 backdrop-blur shadow-2xl border-l border-gray-200 z-50 flex flex-col transform transition-transform animate-in slide-in-from-right duration-300 ease-out">
                    ...
                    <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/80">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={handleSaveDetailChanges} className="text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded transition-colors">Guardar y Cerrar</button>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-x-6 gap-y-4 p-4 bg-gray-50/50 rounded-xl border border-gray-100">
                            <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] uppercase font-bold text-blue-600 tracking-wider">Abogado</span>
                                </div>
                                <div className="flex items-center gap-2">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Fecha de Vencimiento</label>
                                <input
                                    type="date"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Prioridad</label>
                                <select
                                ...
                                >
                                    {Object.values(TaskPriority).map(p => <option key={p} value={p}>{p === 'Urgent' ? 'Urgente' : p === 'High' ? 'Alta' : p === 'Low' ? 'Baja' : 'Media'}</option>)}
                                </select>
                            </div>

                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Cliente Vinculado</label>
                                <select
                            ...
                            <div className="col-span-2 space-y-1">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Categoría Legal</label>
                                <select
                        ...
                        <div>
                            <h4 className="text-xs font-bold text-gray-900 mb-3 flex items-center gap-2 uppercase tracking-wide">
                                <Icons.CheckSquare size={14} className="text-gray-400" /> Subtareas
                            </h4>
                        ...
                        <div>
                            ...
                            <div className="space-y-4 mb-4">
                                ...
                                {(!selectedTask.comments || selectedTask.comments.length === 0) && (
                                    <p className="text-xs text-gray-400 italic">Aún no hay comentarios.</p>
                                )}
                            </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl">
                        <div className="flex justify-between items-center mb-5">
                            <h2 className="text-lg font-bold text-gray-900">Crear Nueva Tarea</h2>
                        ...
                        <form onSubmit={handleCreateTask} className="space-y-4">
                            <div>
                                ...
                                <input name="title" required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-900 focus:outline-none" placeholder="ej. Redactar NDA" />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                                <textarea name="description" rows={3} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-900 focus:outline-none resize-none" placeholder="Agregue detalles..." />
                            </div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Cliente Vinculado (Opcional)</label>
                            ...
                            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                                <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg">Cancelar</button>
                                <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 rounded-lg">Crear Tarea</button>
                            </div>
                        </form>
                    </div>
                </div>
